...
(function() {
	...
	var c = function(a, b) {
		...
		for ( var m = 0; m < a.length; m += 16) {
			...
			for ( var s = 0; s < 80; s++) {
				s < 16 ? c[s] = a[m + s] : c[s] = g(c[s - 3] ^ c[s - 8]
						^ c[s - 14] ^ c[s - 16], 1);
		...
		return a < 20 ? b & c | ~b & d : a < 40 ? b ^ c ^ d : a < 60 ? b & c
				| b & d | c & d : b ^ c ^ d
	}, e = function(a) {
		return a < 20 ? 1518500249 : a < 40 ? 1859775393 : a < 60 ? -1894007588
				: -899497514
	}, f = function(a, b) {
		var c = (a & 65535) + (b & 65535), d = (a >> 16) + (b >> 16)
				+ (c >> 16);
		return d << 16 | c & 65535
	}, g = function(a, b) {
		return a << b | a >>> 32 - b
		...
		for ( var e = 0; e < b.length * 4; e++)
			d += c.charAt(b[e >> 2] >> (3 - e % 4) * 8 + 4 & 15)
					+ c.charAt(b[e >> 2] >> (3 - e % 4) * 8 & 15);
		return d
	};
	this.base64 = {
		encode : function(a) {
			...
			do {
				...
				b = b + this._keys.charAt(f) + this._keys.charAt(g)
						+ this._keys.charAt(h) + this._keys.charAt(i);
				c = d = e = "";
				f = g = h = i = ""
			} while (j < a.length);
			return b
...
f.setPublic(rasPubkey, "10001");
var sp = f.encrypt([servertime, nonce].join("\t") + "\n" + password);
